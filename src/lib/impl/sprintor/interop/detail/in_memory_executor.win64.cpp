...
inline BOOL IsImportByOrdinal(std::uint64_t ImportDescriptor) {
  return (ImportDescriptor & IMAGE_ORDINAL_FLAG64) != 0;
}

inline HMODULE MemLoadExe(PBYTE data, LPSTR lpCmdLine, bool isWinMain = true) {
  ...
  for (; pImportDscrtr->Name != 0; pImportDscrtr++) {
    ...
    HANDLE *pImport = NULL, *pAddress = NULL;
    HANDLE ProcAddress;

    pAddress = (HANDLE *)(ImageBase + pImportDscrtr->/*Original*/ FirstThunk);
    if (pImportDscrtr->TimeDateStamp == 0)
      pImport = (HANDLE *)(ImageBase + pImportDscrtr->FirstThunk);
    else
      pImport = (HANDLE *)(ImageBase + pImportDscrtr->OriginalFirstThunk);
    for (i = 0; pImport[i] != 0; i++) {
      if (IsImportByOrdinal((std::uint64_t)pImport[i]))
        ProcAddress = (HANDLE)GetProcAddress(
            hLibModule, (LPCSTR)(((std::uint64_t)pImport[i]) & 0xFFFF));
      else // import by name
      {
        pImortName = (PCHAR)(ImageBase + (std::uint64_t)(pImport[i]) + 2);
        ProcAddress = (HANDLE)GetProcAddress(hLibModule, pImortName);
      }
